export default function ProjectCard({ projectData }) {
  ...
  return (
    <article className="relative group flex flex-col sm:flex-row gap-6 sm:gap-12 border border-[#5810A2]/20 hover:border-[#5810A2] p-4 rounded-md bg-[#5810A2]/20 sm:bg-[#5810A2]/20  backdrop-blur sm:group-hover/list:opacity-15 sm:hover:!opacity-100 transition-opacity ">
      {!isImageEmpty && (
        <picture className="">
          <img
            src={imgURL}
      ...
      <div className="flex flex-col gap-6 justify-between">
        <div className="flex flex-col gap-2">
          <h2 className=" text-white font-medium sm:text-lg group-hover:underline group-hover:text-[#B080FF] group-hover:underline-offset-4 ">
            <div>
              <a href={website} target="_blank">
                <span className="absolute inset-x-0 inset-y-0 hidden sm:block"></span>
                <span className="flex flex-row gap-3 sm:justify-between">
                  {header}
                  <svg
